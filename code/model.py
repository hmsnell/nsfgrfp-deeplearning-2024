...
from autoencoder import VAE, CVAE, reparametrize, loss_function
...
def train_vae(model, train_loader, args, is_cvae = True):
    """
    ...
    for (batch, (images, labels)) in enumerate(tqdm(train_loader)):
        with tf.GradientTape() as tape:
            if is_cvae:
                ...
                x_hat, mu, logvar = model(images, onehot_labels)
            else:
